...
parser.add_argument('--num_bits_per_vector', type=int, default=4)
...
outFile=open("eval/ablation/"+args.log_file_name,"w")
outFileEval=open("eval/ablation/"+args.log_file_name+"_","w")
...
for i in range(100):
	if convergence_on_multi_task is None:
		performance_on_multi_task = multi_task_error
		generalization_from_multi_task = [0]#eval_generalization()
	...
	print('{0}'.format(' '.join(map(str, generalization_from_target_task))),file=outFileEval)
